...
{
    /// <summary>
    /// Implementation of <see cref="IAssert"/>.
    /// </summary>
        ...
        public IAssert Equal<T>([AllowNull] T expected, [AllowNull] T actual, IEqualityComparer<T?> comparer)
        {
                                                              ...
                                                              && !aec.Equals(expected, actual, out var mismatchedIndex))
            {
                if (mismatchedIndex.HasValue)
                    throw Mettle.Sdk.EqualException.FromEnumerable(expectedAsIEnum, actualAsIEnum, mismatchedIndex.Value);
                else
                    throw new Mettle.Sdk.EqualException(expected, actual);
            }
